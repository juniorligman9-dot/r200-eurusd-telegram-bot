...
def process_telegram_commands():
    ...
    for update in updates:
        ...
        # If no chat ID was configured, the first user to
        # send /start becomes the authorized user.
        if ALLOWED_CHAT_ID is None:
            ALLOWED_CHAT_ID = int(chat_id)

            logger.info(
                "Telegram chat authorized: %s",
                ALLOWED_CHAT_ID
            )

            send_message(
                ALLOWED_CHAT_ID,
                "🔐 This Telegram chat is now authorized."
            )

        if not command_allowed(chat_id):
            continue
        ...
        if command == "/start":
            send_message(
                chat_id,
                "🤖 R200 EURUSD Trading Bot\n\n"
                "Commands:\n"
                "/status - Account and trade status\n"
                "/on - Enable trading\n"
                "/off - Disable new trades\n"
            ...
            send_message(
                chat_id,
                "🟢 Trading enabled.\n"
                "The bot will only trade when all risk checks pass."
            )
            ...
            if signal is None:
                send_message(
                    chat_id,
                    "⏳ No valid EURUSD signal right now."
                )
            else:
                send_message(
                    chat_id,
...
def get_latest_closed_candle_time():
    ...
    # Candle index -2 is the most recently completed candle.
...
def trading_cycle():
    ...
    # Only evaluate once per completed candle.
    ...
    logger.info(
        "New completed M15 candle detected."
    )

    signal = get_signal()

    if signal is None:
        logger.info("No valid trading signal.")
        return
...
def main():
    logger.info("=" * 60)
    logger.info("R200 EURUSD TELEGRAM TRADING BOT")
    logger.info("=" * 60)
    ...
    broadcast(
        "🟢 R200 EURUSD Bot is online.\n\n"
        "Market: EURUSD\n"
        "Timeframe: M15\n"
        "Risk: 1% per trade\n"
        "Max positions: 1\n\n"
        "Use /status to check the account."
    )
